//Q4)A program to demonstrate chat application based on sockets--server side
...
if (!fork())//fork off a child process to receive messages
{
	...
	break;
        ...
        /* ... and play it */
        if (pa_simple_write(s1, buf, (size_t) r, &error) < 0) {
	...
	/*printf("cleanup before exiting child\n");
	close(new_fd);
	//close(sockfd);
	exit(0);*/
